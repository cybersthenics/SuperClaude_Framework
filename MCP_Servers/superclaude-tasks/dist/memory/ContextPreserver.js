...
export class ContextPreserver {
    constructor(memoryManager, snapshotDir = './data/snapshots') {
    ...
    getSnapshotFilePath(projectId, snapshotId) {
        return path.join(this.snapshotDir, `${projectId}_${snapshotId}.snapshot`);
    }
    // Create context snapshot
    async createContextSnapshot(projectId, contextData, description = 'Context snapshot', options = {}) {
        try {
            ...
            throw new ValidationError(`Failed to create context snapshot: ${error.message}`);
        }
    }
    // Load context snapshot
    async loadContextSnapshot(projectId, snapshotId) {
        try {
            const filePath = this.getSnapshotFilePath(projectId, snapshotId);
            ...
            throw new ValidationError(`Failed to load context snapshot: ${error.message}`);
        }
    }
    // List context snapshots
    async listContextSnapshots(projectId) {
        try {
            const memoryState = await this.memoryManager.loadProjectMemory(projectId);
            ...
            throw new ValidationError(`Failed to list context snapshots: ${error.message}`);
        }
    }
    // Delete context snapshot
    async deleteContextSnapshot(projectId, snapshotId) {
        try {
            ...
            throw new ValidationError(`Failed to delete context snapshot: ${error.message}`);
        }
    }
    // Serialize context data
    async serializeContextData(contextData, options = {}) {
        try {
            const finalOptions = { ...this.defaultOptions, ...options };
            ...
            throw new ValidationError(`Failed to serialize context data: ${error.message}`);
        }
    }
    // Deserialize context data
    async deserializeContextData(serializedData, compressed = false) {
        try {
            ...
            throw new ValidationError(`Failed to deserialize context data: ${error.message}`);
        }
    }
    // Create incremental snapshot
    async createIncrementalSnapshot(projectId, contextData, baseSnapshotId, description = 'Incremental snapshot') {
        try {
            ...
            return await this.createContextSnapshot(projectId, incrementalData, description);
        }
        catch (error) {
            this.logger.error(`Failed to create incremental snapshot:`, error);
            throw new ValidationError(`Failed to create incremental snapshot: ${error.message}`);
        }
    }
    // Restore from incremental snapshots
    async restoreFromIncrementalSnapshots(projectId, snapshotIds) {
        try {
            let restoredData = [];
            ...
            throw new ValidationError(`Failed to restore from incremental snapshots: ${error.message}`);
        }
    }
    // Get snapshot statistics
    async getSnapshotStatistics(projectId) {
        try {
            ...
            throw new ValidationError(`Failed to get snapshot statistics: ${error.message}`);
        }
    }
    ...
    calculateIncrementalChanges(baseData, newData) {
    ...
    mergeContextData(base, incremental) {
        const merged = [...base];
        for (const item of incremental) {
            // Simple merge - in practice would use more sophisticated merging
            const existingIndex = merged.findIndex(existing => existing.type === item.type &&
