...
export class BuilderMCPServer {
    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                ...
                throw new McpError(ErrorCode.InternalError, `Tool ${name} failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
            }
        });
    }
    ...
    async handleExtractMethod(args) {
    ...
    async handleExtractFunction(args) {
        const parsed = ExtractMethodSchema.parse(args);
        const selection = {
            uri: parsed.uri,
            range: parsed.selection,
    ...
    async handleGenerateCode(args) {
        ...
        const context = {
            ...
            framework: parsed.context.framework ? { name: parsed.context.framework } : null,
    ...
    async handleGenerateUIComponent(args) {
    ...
    async handleApplyDesignPattern(args) {
    ...
    async handleImplementInterface(args) {
