...
export class ChainModeManager {
     ...
     */
    async startChain(configuration, initialContext) {
        ...
        const contextSnapshotId = await this.contextPreserver.preserveContext(chainId, initialContext, { phase: 'initial', persona: 'none' });
     ...
     */
    configurePersona(persona, configuration) {
     ...
     */
    addHandoffStrategy(name, strategy) {
    ...
    initializePersonaConfigurations() {
        const personas = [
    ...
    initializeHandoffStrategies() {
        ...
        this.handoffStrategies.set('sequential', {
            handoff: async (fromLink, toLink, execution) => {
                return {
                    handoffId: uuidv4(),
    ...
    getDefaultPersonaConfiguration(persona) {
        const baseConfig = {
            timeoutMs: 30000,
    ...
    async createChainLinks(configuration, initialContext) {
        const links = [];
    ...
    async executePersonaLink(link, execution) {
        const startTime = Date.now();
    ...
    async simulatePersonaExecution(persona, input, config) {
    ...
    async performContextHandoff(fromLink, toLink, execution) {
        ...
        const transformedContext = await this.transformContext(execution.context, handoff, fromLink, toLink);
        ...
        const contextSnapshotId = await this.contextPreserver.preserveContext(execution.chainId, transformedContext, {
            phase: 'handoff',
            fromPersona: fromLink.persona,
            toPersona: toLink.persona,
    ...
    async transformContext(originalContext, handoff, fromLink, toLink) {
    ...
    async completeChain(execution) {
        ...
        this.performanceTracker.recordChainCompletion(execution.chainId, execution.results.length, totalTime, this.calculateChainQuality(execution));
    }
    aggregateChainOutputs(execution) {
    ...
    aggregateFindings(outputs) {
        const allFindings = new Set();
        for (const output of outputs) {
            if (output && output.findings) {
                output.findings.forEach((finding) => allFindings.add(finding));
            }
        }
        return Array.from(allFindings);
    }
    aggregateRecommendations(outputs) {
        const allRecommendations = new Set();
        for (const output of outputs) {
            if (output && output.recommendations) {
                output.recommendations.forEach((rec) => allRecommendations.add(rec));
            }
        }
        return Array.from(allRecommendations);
    }
    calculateChainQuality(execution) {
    ...
    calculateAverageLinkTime(execution) {
        if (execution.results.length === 0)
            return 0;
    ...
    calculateHandoffOverhead(execution) {
        if (execution.results.length === 0)
            return 0;
    ...
    calculateAverageExecutionTime(chains) {
        if (chains.length === 0)
            return 0;
    ...
    calculatePersonaUsageDistribution(chains) {
        const distribution = {};
    ...
    calculateHandoffEfficiency(chains) {
        if (chains.length === 0)
            return 0;
        let totalHandoffs = 0;
...
//# sourceMappingURL=ChainModeManager.js.map