...
export class HybridOrchestrator {
    ...
    loopController;
    chainManager;
    constructor(waveEngine, delegationEngine, loopController, chainManager) {
     ...
     */
    async executeHybrid(configuration, context) {
     ...
     */
    async executePattern(pattern, context, config) {
        switch (pattern.type) {
            case 'wave':
    ...
    async executeWavePattern(pattern, context) {
        const operation = {
            type: 'hybrid_wave',
            complexity: 0.7,
            fileCount: 30,
    ...
    async executeDelegationPattern(pattern, context) {
        const task = {
    ...
    async executeLoopPattern(pattern, context) {
        const config = {
            mode: 'enhance',
            maxIterations: 3,
    ...
    async executeChainPattern(pattern, context) {
        const config = {
    ...
    mergeContextWithResults(context, result) {
        return {
            ...context,
            metadata: {
                ...context.metadata,
...
//# sourceMappingURL=HybridOrchestrator.js.map