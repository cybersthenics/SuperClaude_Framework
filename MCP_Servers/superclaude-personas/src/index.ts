...
// SuperClaude Personas - Main MCP Server
// Behavioral Intelligence Engine for SuperClaude MCP ecosystem
...
const config: PersonasServerConfig = {
...
class SuperClaudePersonasServer {
  ...
  constructor() {
    ...
    this.collaborationCoordinator = new CollaborationCoordinator(
      personaRegistry.getAllPersonas(),
      this.logger,
      this.performanceMonitor
    );
    this.chainModeHandler = new ChainModeHandler(
      personaRegistry.getAllPersonas(),
      this.logger,
      this.performanceMonitor,
      this.collaborationCoordinator
    );
  ...
  private async handleActivatePersona(args: any): Promise<any> {
    ...
    const result = await this.personaManager.activatePersona(
      validated.persona,
      validated.context,
      validated.options
    );

    this.performanceMonitor.recordPersonaActivation(
      validated.persona,
      result.metadata.activationTime,
  ...
  private async handleGetPersonaRecommendation(args: any): Promise<any> {
    ...
    const recommendations = await this.personaManager.getPersonaRecommendation(
      context,
      validated.options
    );
  ...
  private async handleApplyPersonaBehavior(args: any): Promise<any> {
    ...
    const context: PersonaContext = validated.context || {
  ...
  private async handleCoordinatePersonas(args: any): Promise<any> {
    const validated = schemas.CoordinatePersonasSchema.parse(args);
    
    const result = await this.personaManager.coordinateMultiPersona(
      validated.personas,
      validated.operation,
      validated.coordinationMode
    );

    this.performanceMonitor.recordCollaboration(
      validated.personas,
      validated.coordinationMode || 'parallel',
  ...
  private async handleGetPersonaPriorities(args: any): Promise<any> {
    ...
    const priorities = await this.personaManager.getPersonaPriorities(
      validated.persona,
      validated.decisionContext
    );
    ...
    if (validated.options?.comparativeAnalysis) {
      comparison = await this.generatePersonaComparison(
        validated.persona,
        validated.options.comparativeAnalysis
      );
    }
  ...
  private async handleShareExpertise(args: any): Promise<any> {
    const validated = schemas.ShareExpertiseSchema.parse(args);
    
    const expertise: ExpertiseContribution = {
      ...validated.expertise,
      timestamp: validated.expertise.timestamp || new Date()
    };

    const result = await this.personaManager.shareExpertise(
      validated.fromPersona,
      validated.toPersona,
      expertise
    );
  ...
  private getCollaborationPatterns(): any {
    const patterns: any = {};
  ...
  private getActivationTriggers(): any {
    const triggers: any = {};
  ...
  private async generatePersonaComparison(
    persona: PersonaName,
    compareWith: PersonaName[]
  ): Promise<any> {
  ...
  private comparePriorities(priorities1: string[], priorities2: string[]): any {
  ...
  private compareMcpPreferences(prefs1: any[], prefs2: any[]): any {
  ...
  private compareDomains(strategies1: any[], strategies2: any[]): any {
