...
class BasicPersonasServer {
  ...
  private async handleCoordinatePersonas(args: any): Promise<any> {
    ...
    for (const persona of personas) {
      if (!PERSONAS[persona as keyof typeof PERSONAS]) {
        throw new McpError(ErrorCode.InvalidRequest, `Persona ${persona} not found`);
      }
    }
    ...
    const topPersona = results.reduce((prev, current) => 
  ...
  private identifyPotentialConflicts(results: any[]): string[] {
    const conflicts = [];
    
    // Check for priority conflicts
    const priorities = results.map(r => PERSONAS[r.persona as keyof typeof PERSONAS].priorityHierarchy[0]);
    ...
    const mcpServers = results.flatMap(r => 
      PERSONAS[r.persona as keyof typeof PERSONAS].mcpPreferences.map(p => p.serverName)
    );
