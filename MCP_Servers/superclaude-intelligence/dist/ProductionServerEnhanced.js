...
class ProductionEnhancedServer {
    ...
    async handleFindSymbolDefinition(args) {
        ...
        if (this.lspManager instanceof EnhancedLSPManager) {
            result = await this.lspManager.sendRequestEnhanced(language, 'textDocument/definition', {
    ...
    async handleGetDocumentSymbols(args) {
        const startTime = performance.now();
        ...
        let result;
        if (this.lspManager instanceof EnhancedLSPManager) {
            result = await this.lspManager.sendRequestEnhanced(language, 'textDocument/documentSymbol', {
                textDocument: { uri: args.uri }
            });
        }
        else {
    ...
    async handleFindAllReferences(args) {
        ...
        let result;
        if (this.lspManager instanceof EnhancedLSPManager) {
            result = await this.lspManager.sendRequestEnhanced(language, 'textDocument/references', {
                textDocument: { uri: args.uri },
                position: args.position,
            ...
            result = await this.lspManager.findReferences(args.uri, args.position, args.includeDeclaration !== false);
        }
        const duration = performance.now() - startTime;
        return {
            content: [
    ...
    async handleGetHoverInfo(args) {
        ...
        let result;
        if (this.lspManager instanceof EnhancedLSPManager) {
            result = await this.lspManager.sendRequestEnhanced(language, 'textDocument/hover', {
                textDocument: { uri: args.uri },
                position: args.position
            });
        }
    ...
    async handleGetCodeCompletions(args) {
        ...
        let result;
        if (this.lspManager instanceof EnhancedLSPManager) {
            result = await this.lspManager.sendRequestEnhanced(language, 'textDocument/completion', {
                textDocument: { uri: args.uri },
                position: args.position
            });
        }
    ...
    async handleBatchLSPRequests(args) {
        if (!(this.lspManager instanceof EnhancedLSPManager)) {
    ...
    async handleGetLSPMetrics(args) {
        let metrics = {
    ...
    async handleOptimizeLSPConnections(args) {
        if (!(this.lspManager instanceof EnhancedLSPManager)) {
    ...
    async handleIncrementalUpdate(args) {
        const startTime = performance.now();
        if (this.lspManager instanceof EnhancedLSPManager) {
